...
from tests.basetest import BaseTest, git_repo, envvar, chdir
...
def test_get_build_id(mock_gmtime: MagicMock):
    ...
    with git_repo() as git_root, envvar("AUTOBUILD_BUILD_ID", None):
        ...
        build_id = common.get_build_id(config)
    ...
    with tempfile.TemporaryDirectory() as dir, chdir(dir), envvar("AUTOBUILD_BUILD_ID", None):
        config = mock_config(os.path.join(dir, "autobuild.xml"))
        build_id = common.get_build_id(config)
        assert build_id == want_build_id
    
    # Finally, check that the AUTOBUILD_BUILD_ID is used by default
    with tempfile.TemporaryDirectory() as dir, chdir(dir), envvar("AUTOBUILD_BUILD_ID", "50000"):
        config = mock_config(os.path.join(dir, "autobuild.xml"))
        build_id = common.get_build_id(config)
...
def test_establish_build_id():
    with tempfile.TemporaryDirectory() as dir, envvar("AUTOBUILD_BUILD_ID", None):
        config = mock_config(os.path.join(dir, "autobuild.xml"))
        build_id = common.establish_build_id("50000", config)
        assert build_id == "50000"
